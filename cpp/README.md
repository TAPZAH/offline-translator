...
установленный каталог. Каталог доступных пакетов сейчас встроенный: один
пакет NLLB (`mijuanlo/nllb-200-distilled-600M-ct2-int8`) и пары Argos
`en↔ru`. `update_remote_index()` — заглушка, удалённый индекс Argos на
этом этапе не скачивается.
...
Окно «Пакеты» показывает статус NLLB и пар Argos, ставит/снимает пакеты
через менеджеры моделей. Прогресс установки тоже идёт через `PostMessage`.
Удаление NLLB требует подтверждения.
