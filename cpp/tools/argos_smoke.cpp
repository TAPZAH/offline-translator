...
int main(int argc, char** argv) {
    if (argc != 2) {
        std::cerr << "Использование: argos_smoke <корень-пакетов>\n";
        return 2;
    }

    try {
        offline_translator::ArgosModelManager model_manager(
            argv[1], "en", "ru");
        model_manager.validate();
        offline_translator::ArgosEngine engine(argv[1]);
        ...
        std::cerr << "Ошибка Argos smoke-test: " << error.what() << "\n";
        return 1;
    }
}
